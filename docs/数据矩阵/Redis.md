...
> 围绕“数据结构选择 → 缓存模式 → 一致性 → 高可用”整理 Redis。保留原有命令和项目实践，同时修复不可发布的本机图片引用。

| 项目 | 说明 |
| --- | --- |
| 难度 | 基础到实战 |
| 适合读者 | 需要学习 Redis 数据结构、缓存、持久化和并发业务应用的开发者 |
...
- 掌握 String、Hash、List、Set、ZSet 的适用场景
- 理解过期、淘汰、持久化与缓存异常
- 能够实现分布式锁、计数、排行和库存操作
- 认识主从、哨兵、集群和性能诊断
...
- Lua 脚本在 Redis 内原子执行，避免“读取后再扣减”的竞态。
- 脚本只负责原子状态变更，订单落库仍需幂等和最终一致性设计。
- 集群环境中多个 KEYS 必须位于同一 hash slot。
...
参数校验；对不存在结果设置短期空值；高风险场景使用布隆过滤器；监控异常 key 访问。

### 2. 安全释放分布式锁需要什么条件？

锁值保存唯一持有者 token，释放时通过 Lua 原子比较 token 后删除，避免误删他人的锁。
...
- Redis 原理、命令与项目实践：`数据矩阵/Redis/Redis.md`

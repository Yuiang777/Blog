...
> 以可运行代码为主线，将语法、对象模型、集合、异常、并发基础与运行时机制串联起来。学习时优先掌握常用能力，再进入 JVM 和动态代理。

| 项目 | 说明 |
| --- | --- |
| 难度 | 基础到进阶 |
| 适合读者 | 需要系统复习 Java 语法、集合、面向对象、JVM、反射与动态代理的开发者 |
...
**为什么equal()相等那么hashcode()也要相等？**

hashCode 用于定位元素所在的桶，equals 用于判断对象是否真正相等。在哈希结构中只会在同一个桶内调用 equals，因此如果两个对象 equals 相等，它们的 hashCode 必须相等，否则会被分到不同桶中，导致查找失败。
...
- 线程不安全
- 底层结构：动态数组，连续内存缓存友好
- 中间增删是O(N)：本质是搬移后半段的所有元素
- 随机访问O(1)

**加入频繁中间增加数据怎么优化？**

可以用addAll进行批量增加数据来减少移动的次数，预先分配容量以避免扩容操作，一次性构建插入，或者切换数据结构，如果是有序的就用跳表链表等分块操作的数据结构。

###### LinkedList

- 线程不安全
- 底层结构：双向链表，离散内存，缓存不友好
- 已知位置增删是O(1)，否则就是O(N)
- 随机访问是O(N)
...
- 线程安全
- 基本废弃（因为所有的方法都加了synchronized，读和读之间也互斥）
...
- 金额使用整数分保存，避免浮点精度问题。
- 先处理空集合、空元素和非法金额，再进行聚合。
- 只在数据转换清晰时使用 Stream；复杂分支可以使用普通循环。
...
字段使用 `final`；在构造阶段校验币种和金额；加减运算返回新对象；实现基于值的 `equals` 与 `hashCode`。

### 2. 解释 `HashMap` 为什么要求键的哈希值保持稳定。

键进入桶后依赖哈希定位；若键的字段变化导致哈希变化，原条目可能无法再被正确查找。
...
- Java 语言基础与常用 API：`后端/java/Java基础.md`
- JVM、反射与动态代理：`后端/java/Java进阶.md`
